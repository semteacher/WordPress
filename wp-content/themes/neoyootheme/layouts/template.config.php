...
if ($color != '' && $color != 'default') {
    $this->warp->stylesheets->add("css:variations/{$color}.css");
}
...
if ($config->get('load_googlefonts', false)) {
	...
	if(in_array($color, array('retrobeige','retrolandscape','retropaper'))){
		$this->warp->stylesheets->add($httpmode.'://fonts.googleapis.com/css?family=Yanone+Kaffeesatz:regular,light|Lobster');
	}
	
	if(in_array($color, array('gradientblue','gradientbusiness','gradientred','gradientribbon','gradientspace'))){
		$this->warp->stylesheets->add($httpmode.'://fonts.googleapis.com/css?family=Nobile:regular,bold');
	}
}
...
// add class="nofrontpage" to body tag if not frontpage, category blog or section blog
if ($this->warp->system->isBlog()) {
		$config->set('frontpage', true);
}
if ($this->warp->config->get('custom') == 'nobox') {
	$config->set('frontpage', true);
}
$config->set('frontpage', ($config->get('frontpage',false)) ? '' : 'nofrontpage');